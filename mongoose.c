...
#ifdef MG_ENABLE_LINES
#line 1 "mip/driver_imx_rt1020.c"
#endif
...
static bool mip_driver_imx_rt1020_init(struct mip_if *ifp);
static void wait_phy_complete(void);
static struct mip_if *s_ifp;                         // MIP interface

static size_t mip_driver_imx_rt1020_tx(const void *, size_t , struct mip_if *);
static bool mip_driver_imx_rt1020_up(struct mip_if *ifp);
...
void delay (uint32_t di) {
  uint32_t t0 = mg_millis();
  while (mg_millis() < (t0+di));
}

static void wait_phy_complete(void) {
  delay(1); // ms
  const uint32_t max_loops = 10;
  uint32_t loops = 0;
  while (!(ENET->EIR & BIT(23)) && (loops < max_loops)) {
    loops++;
    delay(1);
  }
...
// static bool mip_driver_imx_rt1020_init(uint8_t *mac, void *data) { // VO
static bool mip_driver_imx_rt1020_init(struct mip_if *ifp) {

  struct mip_driver_imx_rt1020_data *d = (struct mip_driver_imx_rt1020_data *) ifp->driver_data;
  s_ifp = ifp;
  ...
  while (eth_read_phy(PHY_ADDR, PHY_BSR) & BIT(15)) {delay(1);} // Wait finished poll 10ms
...
static size_t mip_driver_imx_rt1020_tx(const void *buf, size_t len, struct mip_if *ifp) {
  ...
  if (eir & EIMR_RX_ERR) // Global mask used
  {
    ...
    // Read inframes
      ...
      for (uint32_t i = 0; i < 10; i++) {  // read as they arrive but not forever
        ...
        mip_qwrite(rx_buffer_descriptor[s_rxno].buffer, len > 4 ? len - 4 : len, s_ifp);
...
struct mip_driver mip_driver_imx_rt1020 = {
  mip_driver_imx_rt1020_init, mip_driver_imx_rt1020_tx, mip_driver_rx,
  mip_driver_imx_rt1020_up};
